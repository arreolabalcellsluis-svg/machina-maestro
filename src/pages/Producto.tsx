...
import { ChevronRight, Download, CheckCircle2, Wrench, ShieldCheck, Check, ChevronLeft, ChevronRight as ChevronRightIcon } from "lucide-react";
...
const Producto = () => {
  ...
  if (isLoading) return <div className="p-12 text-center text-muted-foreground">Cargando producto...</div>;
  if (!product) return <Navigate to="/equipos" />;

  const allImages = [product.imagenPrincipal, ...product.galeria.filter(img => img !== product.imagenPrincipal)].filter(Boolean);
      ...
      <div className="container px-4 py-12">
        <div className="grid lg:grid-cols-2 gap-12 mb-16">
          <div className="space-y-4">
            <div className="bg-secondary/20 border rounded-xl p-8 flex items-center justify-center h-[500px] cursor-zoom-in" onClick={() => openLightbox(selectedImage)}>
              <img src={allImages[selectedImage]} alt={product.nombre} className="max-h-full object-contain mix-blend-multiply" />
            </div>
            {allImages.length > 1 && (
          ...
          <div className="flex flex-col">
            <div className="mb-2 text-primary font-black tracking-widest uppercase text-sm">{product.marca}</div>
            ...
            <div className="space-y-4 mt-auto">
              <Button className="w-full h-14 text-lg font-bold" onClick={() => document.getElementById('cotizar-section')?.scrollIntoView({ behavior: 'smooth' })}>
                SOLICITAR COTIZACIÓN
              </Button>
              <div className="grid grid-cols-2 gap-4">
                {product.fichaTecnicaPdf ? (
                  <a href={product.fichaTecnicaPdf} target="_blank" rel="noopener noreferrer">
                    <Button variant="outline" className="w-full h-12 font-bold border-2">
                      <Download className="mr-2 h-4 w-4" /> FICHA TÉCNICA
                    </Button>
                  </a>
                ) : (
                  <Button variant="outline" className="h-12 font-bold border-2" disabled>
                    <Download className="mr-2 h-4 w-4" /> FICHA TÉCNICA
                  </Button>
                )}
                <Button variant="secondary" className="h-12 font-bold">VER VIDEO</Button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div id="cotizar-section" className="bg-secondary rounded-2xl p-8 md:p-12 mb-20 max-w-4xl mx-auto border shadow-sm">
          ...
          <LeadForm productoPredefinido={`${product.nombre} (Mod. ${product.modelo})`} origen={`Página de Producto: ${product.sku}`} />
        </div>
      </div>
      ...
      <Dialog open={lightboxOpen} onOpenChange={setLightboxOpen}>
        <DialogContent className="max-w-[95vw] max-h-[95vh] p-0 bg-black/95 border-none flex items-center justify-center">
          ...
          <div className="flex items-center justify-center w-full h-[85vh] p-8">
            <img src={allImages[lightboxIndex]} alt={product.nombre} className="max-w-full max-h-full object-contain" />
          </div>
